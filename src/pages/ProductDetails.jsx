        ...
        <div className="grid gap-10 rounded-3xl bg-white p-6 shadow-sm md:grid-cols-2 md:p-10">
          ...
          <div className="flex flex-col justify-center">
            ...
            <div className="mt-8 flex gap-4">
              <button
                onClick={handleAdd}
                className={`cart-action flex-1 rounded-xl px-6 py-4 font-bold text-white ${added ? "is-added" : "bg-blue-600 hover:bg-blue-700"}`}
              >
                {added ? "✓ Added to cart" : "🛒 Add to Cart"}
              </button>

              <button onClick={() => addToWishlist(product)} aria-label={isWishlisted ? "Remove product from wishlist" : "Add product to wishlist"} className={`rounded-xl border-2 px-6 py-4 text-xl transition ${isWishlisted ? "border-red-500 bg-red-50 text-red-600 shadow-md" : "border-red-300 text-red-400 hover:bg-red-50 hover:text-red-500"}`}>
                {isWishlisted ? "♥" : "♡"}
              </button>
            </div>
