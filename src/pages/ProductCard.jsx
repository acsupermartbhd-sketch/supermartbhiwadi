...
function ProductCard({ product, addToCart, addToWishlist, customerSession }) {
  ...
  return (
    <div className="product-card group flex h-full cursor-pointer flex-col overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-gray-200 transition duration-300 hover:-translate-y-1 hover:shadow-xl" onClick={openProduct} onKeyDown={handleCardKeyDown} role="link" tabIndex={0} aria-label={`View details for ${product.name}`}>
      <div className="product-image-frame relative w-full overflow-hidden bg-gray-100">
        <img src={product.image} alt={product.name} loading="lazy" decoding="async" className="product-image h-full w-full object-contain transition duration-500 group-hover:scale-105" />
      ...
      <div className="relative z-10 flex flex-1 flex-col p-5">
        ...
        <button onClick={(event) => { event.stopPropagation(); handleAdd(); }} className={`cart-action mt-auto w-full rounded-xl px-4 py-3 font-semibold text-white transition ${added ? "is-added" : "bg-blue-600 hover:bg-blue-700"}`}><span className={added ? "cart-check" : ""}>{added ? "✓ Added to cart" : "🛒 Add to Cart"}</span></button>
      </div>
    </div>
  );
}
